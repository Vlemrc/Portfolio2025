...
export default function Sidebar() {
  ...
  useEffect(() => {
    ...
    if (pathname === "/about") {
      newTranslateY = isMobile ? 51 : 31
    } else if (pathname === "/path") {
      newTranslateY = isMobile ? 96 : 77
    } else if (pathname === "/contact") {
      newTranslateY = isMobile ? 139 : 122
    }
  ...
  return (
    <>
          ...
          >
            ...
            <div className="flex flex-col gap-8 pb-8 lg:pb-10">
              ...
              <p
                ...
                style={{ color: activeProject?.color }}
              >Web developer <br /> Open to projects
              </p>
            </div>
          </div>
        )}
      ...
      {(!isMobile) && (
        <p className="z-50 text-right text-white uppercase text-[10px] font-normal absolute bottom-8 lg:bottom-10 right-8 lg:right-10 transition-colors duration-300" style={{ color: activeProject?.color }}>
      ...
      {isMobile && burgerOpen && (
        <div className="fixed top-[90px] left-0 w-full h-[calc(100dvh-90px)] backdrop-blur-xl z-40 flex flex-col justify-between">
          <nav className="pt-10 lg:pt-[200px] flex flex-row justify-between">
            ...
            <div
              id="active-nav"
              className="absolute lg:relative h-2 w-2 rounded-full left-10 bg-white transition-transform transition-colors duration-300"
              style={{ transform: `translateY(${translateY}px)`, backgroundColor: activeProject?.color }}
            ></div>
          </nav>
          <div className="flex flex-col gap-8 pb-8 pl-10">
              <Socials />
              <p
                className="text-white uppercase text-[10px] font-normal transition-colors duration-300"
                style={{ color: activeProject?.color }}
